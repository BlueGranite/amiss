...
cat("Starting Step 01: Parse VCF\n")
...
sessionid <-  Sys.getenv("AMISS_SESSION_ID")

dir.create(here("output"))
dir.create(here("output", sessionid))
dir.create(here("output", sessionid, "data"))
...
# vcf_filename <- here("data", "clinvar_20190624.vep.vcf_head_10000")
# cadd_snv_filename <- here("data", "CADD_clingen.tsv")
# cadd_indel_filename <- here("data", "CADD_clingen_indel.tsv")

vcf_filename <- Sys.getenv("AMISS_VCF_FILENAME")
cadd_snv_filename <- Sys.getenv("AMISS_CADD_SNV_FILENAME")
cadd_indel_filename <- Sys.getenv("AMISS_CADD_INDEL_FILENAME")
...
write.csv(vcf_df, here("output", sessionid, "data", FILE_FULL_CLINGEN_CSV))
...
write.csv(file = here("output", sessionid, "data", FILE_MERGED_DATA_CSV), x = merged_data, row.names = FALSE)

write(capture.output(sessionInfo()), here("output", sessionid, "01_parse_vcf_sessioninfo.txt"))
